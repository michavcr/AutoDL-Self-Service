...
a learning curve.
Previous updates:
20200903: [MV] Reversing the competition (to be able to submit datasets)
...
def get_solution_or_None(solution_dir):
  """Get the solution array from solution directory."""
  solution_names = sorted(ls(os.path.join(solution_dir, '*.solution')))
  if len(solution_names) != 1: # Assert only one file is found
    logger.info("{} solution files found: {}! "\
                   .format(len(solution_names), solution_names) +
                   "Return `None` as solution.")
    return None
  solution_file = solution_names[0]
...
class Evaluator(object):
  ...
  def __init__(self, solution_dir=None, prediction_dir=None, score_dir=None,
    ...
    self.no_solution_yet = True
    # Resolve info from directories (see score_new_predictions)
    self.solution = None
    # Check if the task is multilabel (i.e. with one hot label) (see score_new_predictions)
    self.is_multiclass_task = None
  ...
  def get_solution_or_None(self):
    """Get solution as NumPy array from `self.solution_dir`."""
    solution = get_solution_or_None(self.solution_dir)
    logger.debug("Successfully loaded solution from solution_dir={}"\
                 .format(self.solution_dir))
    return solution
  ...
  def score_new_predictions(self):
    if evaluator.no_solution_yet:
      sol = evaluator.get_solution_or_None()
      if sol is None:
        return None
      else:
        evaluator.solution = sol
        evaluator.is_multiclass_task = is_multiclass(evaluator.solution)
        if evaluator.task_name is None:
          evaluator.task_name = get_task_name(evaluator.solution_dir)
        evaluator.no_solution_yet = False

    new_prediction_files = evaluator.get_new_prediction_files()

    if len(new_prediction_files) > 0:
      #After the first predictions, ingestion has moved the *.solution file from the submission to the right directory
      if evaluator.no_solution_yet:
        # Resolve info from directories
        evaluator.solution = evaluator.get_solution()
...
if __name__ == "__main__":
    ...
    logger.debug("-" * 50)
    solution_dir = args.prediction_dir
