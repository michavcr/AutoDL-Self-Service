...
class Model(object):
    ...
    def save(self, output_directory):
        if not(self.model is None or self.model == -1):
            self.model.save(os.path.join(output_directory, "second_stage_model.h5"))
        elif self.last_model is not None:
            print("[NLP] Exporting last model...")
            self.last_model.save(os.path.join(output_directory, "second_stage_model.h5"))
