...
class TfdsConvertor(AbsTfdsConvertor):
    ...
    def init_test_tfds(self, test_tfds, unlabelled_tfds=None):
        if self.test_tfds is None:
            self.test_tfds = test_tfds
            self.unlabelled_tfds = unlabelled_tfds
            ...
            if self.if_padded_batch:
                ...
                if self.unlabelled_tfds is not None:
                    self.unlabelled_tfds = test_tfds.padded_batch(
                        self.padded_batch_size,
                        padded_shapes=([None, 1, 1, 1], [None]),
                        padding_values=(tf.constant(-1, dtype=tf.float32), tf.constant(-1, dtype=tf.float32)),
                    )
                    print("init_test_tfds: unlabelled_tfds is not None")
    ...
    def get_test_np(self):
        print("Entering get_test_np")
        if self.test_tfds is None:
            print("self.test_tfds is None")
            return self.accum_test_x, self.accum_test_y

        if len(self.accum_test_x) == 0:
            print("building accum_test")
            ...
            if self.if_padded_batch:
                print("padded_batch is activated")
                while True:
                    try:
                ...
                self.true_accm_test_cnt = self.accm_test_cnt

                if self.unlabelled_tfds is not None:
                    print("unlabelled_tfds is not None")
                    tfds_unlabelled_os_iterator = self.unlabelled_tfds.make_one_shot_iterator()
                    tfds_unlabelled_iter_next = tfds_unlabelled_os_iterator.get_next()

                    while True:
                        try:
                            example, labels = self.tfds_convertor_sess.run(tfds_unlabelled_iter_next)
                            example = np.squeeze(example, (2, 3))
                            example = np.squeeze(example, axis=-1)
                            example = example.astype(np.int)
            ...
            print("accum test shape: ", self.accum_test_y.shape)
