...
class LogicModel(Model):
    ...
    def build_or_get_dataloader(self, mode, dataset=None, num_items=0, unlabelled=False):
        if (mode in self.dataloaders and self.dataloaders[mode] is not None) and not unlabelled:
    ...
    def test(self, dataset, remaining_time_budget=None, unlabelled=False):
        ...
        if unlabelled:
            test_metadata_filename = self.metadata.get_dataset_name().replace('train', 'unlabelled') + '/metadata.textproto'
            print(test_metadata_filename)
            num_test = [int(line.split(':')[1]) for line in open(test_metadata_filename, 'r').readlines()[:3] if
                         'sample_count' in line][0]
        else:
