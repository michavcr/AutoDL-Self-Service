...
class LogicModel(Model):
    ...
    def build_or_get_dataloader(self, mode, dataset=None, num_items=0, unlabelled=False):
    ...
    def test(self, dataset, remaining_time_budget=None, unlabelled=False):
        if (self.ensembleconfig.MODEL_INDEX == 0) and (self.model == self.model_space[self.ensembleconfig.MODEL_INDEX]):
            rv=self.base_test(dataset,self.model,self.model.checkpoints,remaining_time_budget=None, unlabelled=unlabelled)
        else:
            self.base_test(dataset,self.model,self.model.checkpoints,remaining_time_budget=None, unlabelled=unlabelled)
            rv=self.ensemble_prediction()
        self.history_preds = rv
        return rv

    def base_test(self, dataset, model, checkpoints, remaining_time_budget=None, unlabelled=False):
        ...
        if unlabelled: #CHANGE THE COUNT!
            unlabelled_metadata_filename = self.metadata.get_dataset_name().replace('train', 'unlabelled') + '/metadata.textproto'
            print(unlabelled_metadata_filename)
            num_test = [int(line.split(':')[1]) for line in open(test_metadata_filename, 'r').readlines()[:3] if 'sample_count' in line][0]

        dataloader = self.build_or_get_dataloader('test', dataset, num_test, unlabelled=unlabelled)
