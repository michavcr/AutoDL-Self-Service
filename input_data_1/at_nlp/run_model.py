        ...
        if hasattr(self, "model"):
            if isinstance(self.model, sklearn.base.BaseEstimator):
                ...
                with open(filename, 'wb') as filehandler:
                    pickle.dumps(self.model, filehandler)
                with open(os.path.join(output_directory, "info_model.txt"), 'w') as info_file:
                    info_file.write("AutoNLP\nScikit-learn model\nsklearn version " + sklearn.__version__)

            else:
                self.model.save(os.path.join(output_directory, "model.h5"))
                with open(os.path.join(output_directory, "info_model.txt"), 'w') as info_file:
                    import keras
                    info_file.write("AutoNLP\nKeras model\nkeras version " + keras.__version__)

        self.second_stage_model.save(output_directory)