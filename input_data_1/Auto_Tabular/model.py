...
class Model(object):
  ...
  def test(self, dataset, remaining_time_budget=None, unlabelled=False, unlabelled_data=None):
    ...
    if self.pre_increament_preds or self.first_preds:
      if self.X_test is None:
        ...
        if unlabelled_data is not None:
          print("Unlabelled data is provided.")
          X_unlabelled, _ = self.to_numpy_test(unlabelled_data)
          print("X_test previous shape: ", self.X_test.shape)
          print("X_unlabelled current shape: ", X_unlabelled.shape)
          self.X_test = np.concatenate((self.X_test, X_unlabelled), axis=0)
          print("X_test final shape: ", self.X_test.shape)

        self.X_test = pd.DataFrame(self.X_test)

      print(self.Y_train)
      print("y = ", self.Y_train.shape[1])
    ...
    """if unlabelled:
      preds = preds[self.num_test_samples:]
    else:
      preds = preds[:self.num_test_samples]
    """
    return preds, self.num_test_samples
  ...
  def save(self, output_directory):
    ...
    if self.explore is not None:
        self.explore.save(output_directory)
