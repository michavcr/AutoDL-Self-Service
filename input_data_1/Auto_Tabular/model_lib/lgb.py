...
class LGBModel(MetaModel):
    ...
    def save(self, output_directory):
        if not is_multi_label:
            lgb.save(self._model, os.path.join(output_directory, "model.txt"))
            with open(os.path.join(output_directory, "info_model.txt"), 'w') as info_file:
                info_file.write("Lightgbm Model\n version " + lgb.__version__)
        else:
            for cls in range(self.num_class):
                lgb.save(self.models[cls], os.path.join(output_directory, "model_" + str(cls) + ".txt"))
