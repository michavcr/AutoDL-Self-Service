...
class XGBModel(MetaModel):
    ...
    def save(self, output_directory):
        if not self.is_multi_label:
            filename = os.path.join(output_directory, "model.pkl")
            with open(filename, 'wb') as filehandler:
                pickle.dumps(self._model, filehandler)
        else:
            for cls in range(self.num_class):
                filename = os.path.join(output_directory, "model_" + str(cls) + ".pkl")
                with open(filename, 'wb') as filehandler:
                    pickle.dumps(self.models[cls], filehandler)
        
        with open(os.path.join(output_directory, "info_model.txt"), 'w') as info_file:
            info_file.write("Xgb Model\n version " + xgb.__version__)
