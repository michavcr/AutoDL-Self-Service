...
class LogisticRegression(MetaModel):
    ...
    def save(self, output_directory):
        filepath = os.path.join(output_directory, "model.pkl")
        with open(filepath, "wb") as filehandler:
            pickle.dumps(self.model, filehandler)

        with open(os.path.join(output_directory, "info_model.txt")) as info_file:
            info_file.write("Scikit-learn LogisticRegression Model \n version " + sklearn.__version__)
