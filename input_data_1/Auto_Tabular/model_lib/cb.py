...
class CBModel(MetaModel):
    ...
    def save(self, output_directory):
        if not self.is_multi_label:
            self._model.save_model(os.path.join(output_directory, "model.cbm"))
            with open(os.path.join(output_directory, "info_model.txt"), "w") as info_file:
                info_file.write("CatBoostClassifier\nMultilabels\n version " + catboost.__version__)
        else:
            for cls in range(self.num_class):
                self.models[cls].save_model(os.path.join(output_directory, "model_" + str(cls) + ".cbm"))
            with open(os.path.join(output_directory, "info_model.txt"), "w") as info_file:
                info_file.write("CatBoostClassifier\n" + str(self.num_class) + " binary models \n version " + catboost.__version__)
