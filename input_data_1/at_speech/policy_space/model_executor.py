...
class ModelExecutor:
    ...
    def test_pipeline(self, test_tfds, unlabelled_data=None):
        self.tfds_convertor.init_test_tfds(test_tfds, unlabelled_tfds=unlabelled_data)
    ...
    def save(self, output_directory):
        print("Save Model Executor")
        if self.tr34_cls is not None:
            self.tr34_cls.save(output_directory)
        if self.lr_libl_cls is not None:
            self.lr_libl_cls.save(output_directory, self.is_multilabel)
        if self.lr_sag_cls is not None:
            self.lr_sag_cls.save(output_directory, self.is_multilabel)
