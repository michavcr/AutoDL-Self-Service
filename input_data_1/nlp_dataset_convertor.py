...
class TfDatasetsConvertor(object):
    ...
    def init_test_tfds(self, test_tfds, pad_num=20, unlabelled_tfds=None):
        ...
        if len(self.accum_test_x) == 0:
            ...
            with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
                ...
                while True:
                    try:
                        ...
                        break
                self.real_test_num=self.accm_test_cnt
            ...
            if self.unlabelled_tfds is not None:
                ...
                with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
                    ...
                    while True:
                        try:
                            example, labels = sess.run(tfds_unlabelled_iter_next)
                            example = np.squeeze(example, (2, 3))
                            example = np.squeeze(example, axis=-1)
                            example = example.astype(np.int)
                            X.extend(example)
    ...
    def get_nlp_test_dataset(self, pad_num=20):
        ...
        if self.unlabelled_tfds is not None:
            self.unlabelled_tfds = self.unlabelled_tfds.padded_batch(pad_num, padded_shapes=([None, 1, 1, 1], [None]),
                                                                     padding_values=(
                                                                        tf.constant(-1, dtype=tf.float32)
                                                                        , tf.constant(-1, dtype=tf.float32)))
