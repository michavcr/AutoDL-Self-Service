...
def copy_solution(dataset_dir, output_dir):
  """Get the solution array from solution directory."""
  solution_names = sorted(ls(os.path.join(dataset_dir, '*.solution')))

  solution_files = solution_names
  
  for f in solution_files:
    filepath = shutil.copy(f, output_dir)
...
if __name__=="__main__":
    ...
    if os.path.exists(os.path.join(dataset_dir, basename, "unlabelled")):
      ...
      D_predict = AutoDLDataset(os.path.join(dataset_dir, basename, "unlabelled"))
    ...
    try:
      ...
      while(not (use_done_training_api and M.done_training)):
        ...
        Y_pred = M.test(D_test.get_dataset(),
                        remaining_time_budget=remaining_time_budget,
                        unlabelled=False,
                        unlabelled_data=D_predict_data)
    ...
    #Predicting on unlabelled data, if exists
    if os.path.exists(os.path.join(dataset_dir, basename, "unlabelled")):
      num_examples_predict = D_predict.get_metadata().size()
      Y_pred = M.test(D_test.get_dataset(),
                      remaining_time_budget=remaining_time_budget,
      ...
      os.makedirs(os.path.join(output_dir, "labelled"))
      filename_test = basename[:-5] + '.predict'
      data_io.write(os.path.join(output_dir,"labelled", filename_test), Y_pred)

    #Checking the pickle
    """with open(model_path,"rb") as filehandler:
      new_dict = pickle.load(filehandler)
      print(new_dict.done_training)"""
